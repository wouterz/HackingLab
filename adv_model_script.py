...
##        print(predDict[i], max(set(predDict[i]), key = predDict[i].count))
    ...
    #a_model.continue_model(500, "./results/adv/weights.500-6.17.hdf5", x_train, y_train, x_valid, y_valid)
...
##        json.dump(adv_layer, outfile)
    ...
    for i in range(0,1):
        ...
        #countDict1F = dict(filter(lambda elem: elem[1]!=0, countDict1.items()))
        #countDict2F = dict(filter(lambda elem: elem[1]!=0, countDict2.items()))
##        for i in range(1112,1312):
##            print(str(np.argmax(output_a[i]))+" <> " +str(np.argsort(newOutput[i])[-2:]))
